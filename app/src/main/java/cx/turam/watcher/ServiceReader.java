...
public class ServiceReader extends Service {
	...
	private List<Map<String, Object>> mListSelected; // Integer		 "pId"
												  // String		 C.pName
												  // Integer	 "work"
												  // Integer	 "workBefore"
		...
		try {
			...
			while (s != null) {
				...
				if (mListSelected != null && !mListSelected.isEmpty()) {
					List<Integer> l = (List<Integer>) (mListSelected.get(0)).get("finalValue");
					if (l != null && l.size() >= maxSamples)
						for (Map<String, Object> m : mListSelected) {
							((List<Integer>) m.get("finalValue")).remove(l.size() - 1);
							((List<Integer>) m.get("pPTD")).remove(((List<Integer>) m.get("pPTD")).size() - 1);
						}
				}
				if (mListSelected != null && !mListSelected.isEmpty()) {
					for (Map<String, Object> m : mListSelected) {
						List<Integer> l = (List<Integer>) m.get("finalValue");
						if (l == null)
							break;
						while (l.size() >= maxSamples)
							l.remove(l.size() - 1);
						l = (List<Integer>) m.get("pPTD");
						while (l.size() >= maxSamples)
							l.remove(l.size() - 1);
					}
				}
			...
			if (mListSelected != null && !mListSelected.isEmpty()) {
				...
				synchronized (mListSelected) {
					...
					for (Map<String, Object> p : mListSelected) {
						try {
							if (p.get("pDead") == null) {
								reader = new BufferedReader(new FileReader("/proc/" + p.get("pId") + "/stat"));
								arrayPIds[n] = Integer.valueOf((String) p.get("pId"));
								++n;
								sa = reader.readLine().split("[ ]+", 18);
								p.put("work", (float) Long.parseLong(sa[13]) + Long.parseLong(sa[14]) + Long.parseLong(sa[15]) + Long.parseLong(sa[16]));
								reader.close();
							}
						} catch (FileNotFoundException e) {
							p.put("pDead", Boolean.TRUE);
							Intent intent = new Intent("cationRemoveProcess");
							intent.putExtra("process", (Serializable) p);
				...
				for (Map<String, Object> entry : mListSelected) {
					List<Integer> l = (List<Integer>) entry.get("pPTD");
					if (l == null) {
						l = new ArrayList<Integer>();
						entry.put("pPTD", l);
					}
					if (entry.get("pDead") == null) {
...
//				Log.d("NativeHeapAllocatedSize", String.valueOf(Debug.getNativeHeapAllocatedSize()/1024));
			...
			if (totalBefore != 0) {
				...
				if (mListSelected != null && !mListSelected.isEmpty()) {
					...
					synchronized (mListSelected) {
						for (Map<String, Object> p : mListSelected) {
							if (p.get("workBefore") == null)
								break;
							l = (List<Float>) p.get("finalValue");
							if (l == null) {
							...
							workPT = (int) ((Float) p.get("work") - (Float) p.get("workBefore"));
	...
	private void record() {
		...
		try {
			if (topRow) {
				...
				if (mListSelected != null && !mListSelected.isEmpty())
					for (Map<String, Object> p : mListSelected)
						sb.append(",").append(p.get("pAppName")).append(" (Pid ").append(p.get("pId")).append(") CPU usage (%)")
						  .append(",").append(p.get("pAppName")).append(" Memory (kB)");
			...
			if (mListSelected != null && !mListSelected.isEmpty())
				for (Map<String, Object> p : mListSelected) {
					if (p.get("pDead") != null)
						sb.append(",DEAD,DEAD");
					else sb.append(",").append(((List<Integer>) p.get("finalValue")).get(0))
							.append(",").append(((List<Integer>) p.get("pPTD")).get(0));
				}
			sb.append(",")
...
//			MediaScannerConnection.scanFile(this, new String[] { mFile.getAbsolutePath() }, null, null);
	...
	void removeProcess(Map<String, Object> process) {
		synchronized (mListSelected) {
			...
			while (i.hasNext())
				if (i.next().get("pId").equals(process.get("pId"))) {
					i.remove();
